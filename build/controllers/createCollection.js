...
const createCollection = (words) => {
    console.log("Función crearColeccion", words);
    if (words.length > 7)
        return [{ post: new RegExp("idiota", "i") }];
    let coleccion = [];
    words.forEach(element => {
        let una = element.toLowerCase();
        while (una.indexOf('á') !== -1)
            una = una.replace('á', 'a');
        while (una.indexOf('é') !== -1)
            una = una.replace('é', 'e');
        while (una.indexOf('í') !== -1)
            una = una.replace('í', 'i');
        while (una.indexOf('ó') !== -1)
            una = una.replace('ó', 'o');
        while (una.indexOf('ú') !== -1)
            una = una.replace('ú', 'u');
        while (una.indexOf('ü') !== -1)
            una = una.replace('ü', 'u');
